...
**Canonical source keys**: pick one key style per manifest and keep to it. Either absolute paths, or wiki-root-relative POSIX paths (`2_Plaud/2026-08-19.md`). Both are supported; the relative style is usually what a wiki's source pages carry as `source_id`, so rewriting those keys to absolute would orphan the references. What must never happen is mixing styles for the same file: `~/vault/file.md` alongside `/home/user/vault/file.md` produces duplicate entries and a re-ingest. Tilde-prefixed keys are the one form that is always wrong, because `~` resolves differently per machine; run `python scripts/manifest.py normalize <manifest-path>` to expand them and merge collision duplicates (keeps newest `ingested_at`, unions `wiki_pages`). `normalize` deliberately leaves relative keys alone. `scripts/manifest.py delta` resolves a walked file against every plausible key spelling, so it works under either style provided it is run from the wiki root.
